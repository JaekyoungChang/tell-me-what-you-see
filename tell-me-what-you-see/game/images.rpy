# Backgrounds
image black = im.Scale("black.png", 1280, 720)
image room = im.Scale("room.jpg", 1280, 720)
image room open = im.Scale("room_window_open.png", 1280, 720)
image room closed = im.Scale("room_window_closed.png", 1280, 720)
image lake = im.Scale("lake.jpg", 1280, 720)
image lake before = im.Scale("lake_before.jpg", 1280, 720)
image lake after = im.Scale("lake_after.jpg", 1280, 720)
image woods = im.Scale("woods.jpg", 1280, 720)
image underwater = im.Scale("underwater.jpg", 1280, 720)
image power plant = im.Scale("nuclear_power_plant.jpg", 1280, 720)
image mind 1 = im.Scale("mind_1.png", 1280, 720)
image mind 2 = im.Scale("mind_2.png", 1280, 720)
image mind 3 = im.Scale("mind_3.png", 1280, 720)
image mind 4 = im.Scale("mind_4.png", 1280, 720)
image mind 5 = im.Scale("mind_5.png", 1280, 720)
image windowscene ugly =  im.Scale("windowscene-bad.png", 1280, 720)  # TODO: replace this
image windowscene pretty =  im.Scale("windowscene-good.png", 1280, 720)  # TODO: replace this

# Boy
image side boy = im.Scale("boy_after_sad.png", 250, 500, xoffset=0, yoffset=200)
image side boy smiling = im.Scale("boy_after_smiling.png", 250, 500, xoffset=0, yoffset=200)
image boy before pose 1 = "boy_before_pose_1.png"
image boy before pose 2 = "boy_before_pose_2.png"
image boy after sad = "boy_after_sad.png"
image boy after smiling = "boy_after_smiling.png"
image boy hospital sad = "boy_hospital_sad.png"
image boy hospital screaming = "boy_hospital_screaming.png"
image boy wakeup = "wakeup_boy.png"
image boy wakeup screaming = "wakeup_boy_screaming.png"

# Girl
image side girl = im.Scale("girl_after_sad.png", 250, 500, xoffset=0, yoffset=200)
image side girl smiling = im.Scale("girl_after_smiling.png", 250, 500, xoffset=0, yoffset=200)
image girl before pose 1 = "girl_before_pose_1.png"
image girl before pose 2 = Placeholder("girl") # TODO: replace this
image girl after sad = "girl_after_sad.png"
image girl after smiling = "girl_after_smiling.png"
image girl hospital sad = "girl_hospital_sad.png"
image girl hospital screaming = "girl_hospital_screaming.png"
image girl wakeup= "wakeup_girl.png"
image girl wakeup screaming = "wakeup_girl_screaming.png"

# Father
image side father = im.Scale("father_pose_1.png", 250, 500, xoffset=0, yoffset=200)
image side father smiling = im.Scale("father_smile.png", 250, 500, xoffset=0, yoffset=200)
image father armcross left = "father_pose_1.png"
image father armcross right= "father_pose_2.png"
image father standing = "father_pose_3.png"

# Jimmy
image jimmy = "jimmy.png"
image jimmy scared = "jimmy-scared.png"

# Heart Monitor
image side heartmonitor = im.Scale("heart_monitor.png", 250, 250, xoffset=0, yoffset=0)
